...
export default function RAGChat({ subjectId, onClose }: Props) {
  ...
  const streamUrl = (import.meta as any).env?.VITE_RAG_STREAM_URL as string | undefined;
  const [pinned, setPinned] = useState<ChatMessage[]>([]);
  const [hljsTheme, setHljsTheme] = useState('github-dark');
  ...
  const streamAnswer = async (query: string, subj?: string) => {
    try {
      ...
      const res = await fetch(streamUrl!, {
        ...
        body: JSON.stringify({ query, subject_id: subj, top_k: 5 }),
  ...
  useEffect(() => {
    try { localStorage.setItem('chat_mini', mini ? '1' : '0'); } catch {}
  }, [mini]);
  ...
  const send = async () => {
    ...
    try {
      ...
      if (streamUrl) {
        ...
        try {
          const resC = await api.ragQuery({ query: queryForModel, subjectId: subjectId ?? undefined, topK: 5 });
          attachContexts(resC.contexts || []);
        } catch {}
      } else {
        const res = await api.ragQuery({ query: queryForModel, subjectId: subjectId ?? undefined, topK: 5 });
  ...
  const exportMarkdown = () => {
    const md = messages.map(m => `${m.role === 'user' ? '## Bạn' : '## Trợ lý'}\n\n${m.content}`).join('\n\n');
  ...
  const useAvatar = (role: 'user' | 'assistant') => role === 'user' ? '🧑‍🎓' : '🤖';
              ...
              <div className="flex items-center justify-between px-4 py-3 border-b border-white/10 bg-white/5">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <div className="text-white font-semibold truncate">Chatbot RAG</div>
                    <div className="text-xs text-white/60 truncate">Hỏi về tài liệu đã tải lên • Cục bộ, riêng tư</div>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  {/* Theme selector */}
                  <select value={hljsTheme} onChange={(e) => setHljsTheme(e.target.value)} className="text-xs bg-white/10 border border-emerald-400/30 text-white/90 rounded px-2 py-1">
                  ...
                  {/* Export */}
                  <button onClick={exportMarkdown} className="text-xs px-2 py-1 rounded-md bg-emerald-500/10 border border-emerald-400/40 text-emerald-100 hover:bg-emerald-500/20">Export MD</button>
                  <button onClick={exportPDF} className="text-xs px-2 py-1 rounded-md bg-emerald-500/10 border border-emerald-400/40 text-emerald-100 hover:bg-emerald-500/20">PDF</button>
                  <button onClick={() => setMini(true)} className="p-2 rounded-md bg-emerald-500/10 border border-emerald-400/40 text-emerald-100 hover:bg-emerald-500/20" title="Thu gọn">➖</button>
                  <button onClick={onClose} className="p-2 rounded-md bg-emerald-500/10 border border-emerald-400/40 text-emerald-100 hover:bg-emerald-500/20" aria-label="Đóng">✕</button>
                </div>
              </div>
              ...
              <div ref={listRef} className="flex-1 overflow-y-auto p-4 space-y-4">
                ...
                {messages.map((m, i) => (
                  <div key={m.id ?? i} className={`flex items-start gap-3 ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                    {m.role === 'assistant' && (
                      <div className="h-8 w-8 flex-none rounded-full bg-emerald-500/20 border border-emerald-400/40 flex items-center justify-center">{useAvatar('assistant')}</div>
                    )}
                    ...
                    {m.role === 'user' && (
                      <div className="h-8 w-8 flex-none rounded-full bg-blue-500/20 border border-blue-400/40 flex items-center justify-center">{useAvatar('user')}</div>
                    )}
                  </div>
                ))}
                {loading && (
